...
class FileUtils {
    companion object {
        ...
        fun clearExistingPlainFiles() {
            for (file in context.filesDir.listFiles().orEmpty()) {
                if (file.path.endsWith(VFS_PLAIN_FILE_EXTENSION)) {
                    Log.w("[File Utils] Found forgotten plain file: ${file.path}, deleting it")
                    deleteFile(file.path)
                }
            }
            for (file in context.getExternalFilesDir(Environment.DIRECTORY_PICTURES)?.listFiles().orEmpty()) {
                if (file.path.endsWith(VFS_PLAIN_FILE_EXTENSION)) {
                    Log.w("[File Utils] Found forgotten plain file: ${file.path}, deleting it")
                    deleteFile(file.path)
                }
            }
            for (file in context.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS)?.listFiles().orEmpty()) {
                if (file.path.endsWith(VFS_PLAIN_FILE_EXTENSION)) {
                    Log.w("[File Utils] Found forgotten plain file: ${file.path}, deleting it")
                    deleteFile(file.path)
                }
        ...
        private suspend fun cleanFilePath(filePath: String?): String? {
            if (filePath != null) {
                ...
                if (filePath.startsWith("content://com.android.contacts/contacts/lookup/")) {
                    ...
                    val result = getFilePath(context, uriToParse)
